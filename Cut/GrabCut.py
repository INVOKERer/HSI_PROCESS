...
def imgGrabCut(file_dir, mask_dir, out_dir):
    ...
    for sp in splits:
        ...
        img_list_A.sort(key=lambda x: str(x.split('_')[4]))
        img_list_B.sort(key=lambda x: str(x.split('_')[4]))

        num_imgs = min(len(img_list_A), len(img_list_B))
        ...
        for n in range(num_imgs):
            ...
            if os.path.isfile(path_A) and os.path.isfile(path_B):
                ...
                I, contours, hierarchy = cv2.findContours(mask2, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
                '''for i, cont in enumerate(contours):
                    ares = cv2.contourArea(cont)   # 计算包围性状的面积
                    length1 = cv2.arcLength(cont, True)
                    length2 = cv2.arcLength(cont, False)
                    ...
                    if M['m00'] < 1000 or ares < 1000 or length1 < 50 or length2 < 50:           # 过滤
                        del contours[i]
                    else:
                ...
                cv2.imwrite(path_A_re, img)


if __name__ == '__main__':
    imgWay = r'E:\HE+CAM5\Cutresult\RGB'      # 原图所在文件夹
    maskway = r'E:\HE+CAM5\Cutresult\gbdtresulta'
    outWay = r'E:\HE+CAM5\Cutresult\GrabCutresulta'
    imgGrabCut(imgWay, maskway, outWay)
